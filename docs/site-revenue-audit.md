...
Technical cleanup from the same dashboard pass: `/reviews/` no longer emits individual self-serving `Review` JSON-LD, and the follow-up cleanup also removes the page-level `AggregateRating` JSON-LD from `/reviews/` after the first GSC validation attempt still saw the page as affected. Visible customer reviews remain in HTML, but the business-owned reviews page is no longer trying to qualify for review-snippet rich results. All public HTML now loads GA4/Clarity through `/analytics.js?v=20260703q`, which blocks external analytics tags on localhost/127.0.0.1. Cloudflare Pages deployment `53f1a6dd` from source `35d8261` is live on the custom domain for the analytics-loader cleanup; the review-rating follow-up needs live deployment and GSC validation. Existing local test sessions will remain in historical Clarity ranges until the date window moves forward, but new local previews should stop polluting production analytics.
