...
Internal-linking fix on 2026-07-05: the six borough/Long Island landing pages were in the sitemap but had `0` raw-HTML inlinks because area navigation was not exposed as static links. Homepage and `/services/` now include visible static links to Manhattan, Brooklyn, Queens, Bronx, Staten Island, and Long Island pages; local and live link-graph validation show each area page now has `2` static inlinks. The post-deploy live crawl covered all `102` sitemap URLs with `0` status/canonical/title/meta/H1/JSON-LD/analytics issues, and IndexNow accepted the updated `102` URL set with HTTP `200 OK`.
