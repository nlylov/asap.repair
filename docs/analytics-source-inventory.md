...
Last verified: 2026-07-04, after sitewide self-serving rating schema cleanup, deferred analytics loading, and homepage Lighthouse retest.
...
| --- | --- | --- | --- |
| Google Analytics 4 | Installed through shared production-only loader | 102 source HTML pages reference `/analytics.js?v=20260704b`; loader defines `gtag()` immediately, then injects `gtag.js?id=G-1ZRVGCMZ43` on first user interaction, lead-event demand, or delayed post-load idle only on `asap.repair` / `www.asap.repair` | Page views, CTA events, quote/chat lead events, paid conversion measurement from CRM |
| Microsoft Clarity | Installed through shared production-only loader | 102 source HTML pages reference `/analytics.js?v=20260704b`; loader defines a queued `clarity()` immediately, then injects Clarity project `wyzjzrud6n` on first user interaction, lead-event demand, or delayed post-load idle only on `asap.repair` / `www.asap.repair` | Session recordings, heatmaps, smart events, bot/session quality, AI Visibility beta |
