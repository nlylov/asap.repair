...
| --- | --- | --- | --- |
| Google Analytics 4 | Installed through shared production-only loader | 102 source HTML pages reference `/analytics.js?v=20260704a`; loader defines `gtag()` immediately, then injects `gtag.js?id=G-1ZRVGCMZ43` after idle/load only on `asap.repair` / `www.asap.repair` | Page views, CTA events, quote/chat lead events, paid conversion measurement from CRM |
| Microsoft Clarity | Installed through shared production-only loader | 102 source HTML pages reference `/analytics.js?v=20260704a`; loader defines a queued `clarity()` immediately, then injects Clarity project `wyzjzrud6n` after idle/load only on `asap.repair` / `www.asap.repair` | Session recordings, heatmaps, smart events, bot/session quality, AI Visibility beta |
...
- Local pre-deploy check on 2026-07-04: representative local URLs (`/`, `/services/ac-installation-cleaning/`, `/reviews/`) returned `200`, referenced `/analytics.js?v=20260704a`, and had no `AggregateRating`, `schema.org/Review`, or review `itemprop` markup. All 102 active runtime pages reference the deferred analytics loader; lab/preview and moved tadelakt alias pages are intentionally excluded from that count.
