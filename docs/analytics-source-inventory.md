...
- Google Search Console review snippets: 97 invalid elements, last updated 2026-07-01. Reported issues were missing `itemReviewed` and invalid `author` object type. Deployment `53f1a6dd` from source `35d8261` removed the 63 individual self-serving `Review` JSON-LD entries from live `/reviews/`, but the first GSC validation attempt still failed because `/reviews/` kept a self-serving `AggregateRating` block. The follow-up source cleanup removed both individual `Review` and `AggregateRating` JSON-LD from `/reviews/`, but the next GSC validation attempt still failed because rendered review cards kept `schema.org/Review` microdata. The source now keeps visible customer reviews as plain HTML only; local checks find 0 `Review` JSON-LD nodes sitewide, no `AggregateRating` on `/reviews/`, no review microdata on `/reviews/`, and 85 remaining service/page `AggregateRating` blocks. GSC will only clear after the microdata cleanup deployment, recrawl, and validation.
