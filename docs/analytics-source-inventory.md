...
- CRM business workflow should make paid conversion measurement deterministic: website/chat lead -> CRM lead/contact -> scheduled job/appointment -> estimate/invoice -> QuickBooks paid status -> CRM paid state -> GA4 `purchase` / `close_convert_lead`. CRM PR #463 covers the missing GA4 lead-stage events in code; business-process discipline still matters so invoices are linked to the right CRM contact/job before payment.
