...
- Runtime JS references now use asset version `20260702b` for `/components/loader.js`, `/main.js`, dynamically loaded quote-modal/module scripts, chat, and related-content. This avoids stale Cloudflare/browser cache serving an older analytics build after deploy. Live verification passed after Cloudflare propagation: the homepage HTML references the versioned loader/main scripts, the loader serves `ASSET_VERSION = '20260702b'`, and a browser smoke saw `phone_click`, `cta_click`, and `quote_modal_open` in `dataLayer`.
